"""
This is the code required for controlling the micromelon rovers using the Emotiv Insight EEGs.
Most of the code will not be used directly for the bot race. Hence, the important parts of the code that you will have to tinker with has been marked out with:
# +++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++ #.

Make sure you have all of the below modules installed, and close the Emotiv BCI software before running the program.
"""
...
import warnings
import threading
from micromelon import * #'pip install micromelon' for install
...
# +++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++ #
    def __init__(self, app_client_id, app_client_secret, **kwargs):
        ...
        try:
            ...
            self.rc.connectBLE(83) #change the number to the designator of your team's rover.
            self.rc.startRover()
        except:
            print("No rover found. make sure you have one connected.")            
...
# +++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++ #
    def handle_command(self, command):
        ...
        if command == 'open':
            if self.c.session_id == '':
                self.profile_name = "MBR 2023 T1"  #change this accordingly to your team
        ...
        # Stream training data (train on EMOTIVBCI first)
            ...
            self.c.bind(new_com_data=self.on_new_com_data)
            time.sleep(2) #display for two seconds
            streams_unbind()
        ...
        elif command == 'move robot':        
            self.c.bind(new_com_data = self.move_robot)
            
            

        elif command == 'connect robot': #only required if you hadn't connected the rover when you started the code.
            self.rc = RoverController()
            self.rc.connectBLE(83) #make sure the designator for your robot is correct.
            self.rc.startRover()
        else:
 ...
 # +++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++ #           
    def move_robot(self, *args, **kwargs):

        """
        this is the main function you will deal with when controlling your robot.
        """
        #the two lines below allow the function to receive data coming from the EEG in real-time. note that the update rate is approx. 100ms.
        data = kwargs.get('data')
        print('mc data: {}'.format(data))


        if data['action'] == 'neutral':
            Motors.moveDistance(0.1)

        """
        elif data['action'] == '--enter type of mental command--':
            pass
        else:
            pass
        
        The "data" variable is a dictionary with three keys: action, power, and time. 
        Action is the type of mental action stored onto this profile after being trained on the emotiv bci software.
        Power is the relative strength of that action. (0-1)
        Time is the timestamp of that action.
        
        As you can see in the if statement above, we have set the rover to move forwards at default speed for 4 cm if there is a strong 'push' signal, and 0.1 cm if weak.
        
        Now it is your job to come up with more elif statements to dictate the movement of the rover. Here are some example functions built into the micromelon library that may be helpful:

        Motors.moveDistance(lDist, lSpeed, rDist=None, rSpeed=None, syncStop=False)
        Motors.setDegreesOffset(offset)
        Motors.turn(speed, secs=None, radius=0, reverse=False)
        Motors.turnDegrees(degrees, speed, radius=0, reverse=False)
        
        More information on the module can be found at: https://micromelon-robotics.github.io/mm-pymodule/micromelon/motors/index.html
        
        Few things to note: 
        - you could theoretically program the robot to travel according to a certain distance dictated by the course, but we recommend against it considering the difficulty of 
        controlling your mental commands.
        - be aware of the time it takes for each movement made by the robot - there will be a new mental data streamed every 0.1s. if each of your command takes too long, there will be
        a significant delay between your mental command and the movement of the rover - better to keep your movements short and consistent. 
        """ 
...
def main():
    ...
    your_app_client_id = "o4Y5Eo1syhACciDflFSPZo1Wc5AU88lU9QeKYVoz"
    your_app_client_secret = "kfN9TxwHoBvW5cSTOAAfMwaH8JApVouScsa4xTNqB8aC4bjeX9atW14X6T0rGD44C43UBGfgbCY5QCZsdxFYf8WqKDBpIbxTbZLjWFQhQ1isBtEeTK0BbSPbZOTA1iji"
